...
type TabId = 'hero' | 'about' | 'contact' | 'work-history' | 'case-studies' | 'timelines';
...
const HeroTab = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.fetchHero' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const saved = await upsertSiteContent('hero', {
        title: name,
        content: heroContent
      }, supabase);
      setContent(saved);
    } catch (err) {
      captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.saveHero' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="mt-8 p-6 bg-background border border-border rounded-lg">
        ...
        <h2 className="text-4xl font-black text-foreground mb-2">{name || 'Your Name'}</h2>
        <p className="font-mono text-lg text-primary mb-3">{title || 'Your Title'}</p>
...
const AboutTab = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.fetchAbout' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const saved = await upsertSiteContent('about', { title, content: body }, supabase);
      setContent(saved);
    } catch (err) {
      captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.saveAbout' });
    } finally {
      setIsSaving(false);
    }
  };
...
const ContactTab = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.fetchContacts' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const resetForm = () => {
    setFormData({ label: '', href: '', value: '', description: '', icon: 'email' });
    setEditingId(null);
    setIsAdding(false);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.saveContact' });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.deleteContact' });
    }
  };
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm text-muted-foreground truncate">{link.href}</div>
              </div>
        ...
        <div className="p-4 bg-muted/50 border border-border rounded-lg space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Icon">
              <Select
                value={formData.icon}
                onChange={(e) => setFormData({ ...formData, icon: e.target.value as ContactIcon })}
                options={iconOptions}
              />
            </Field>
          </div>
...
const WorkHistoryTab = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.fetchWorkHistory' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const resetForm = () => {
    setFormData({ title: '', company: '', period: '', highlights: [], tech: [] });
    setEditingId(null);
    setIsAdding(false);
  };
    ...
    >
                ...
                {entry.highlights.length > 0 && (
                  <ul className="mt-2 text-sm text-muted-foreground list-disc list-inside">
                    ...
                    {entry.highlights.length > 2 && (
                      <li className="text-primary">+{entry.highlights.length - 2} more</li>
                    )}
                  </ul>
                )}
                {entry.tech.length > 0 && (
      ...
      {isAdding ? (
        <div className="p-4 bg-muted/50 border border-border rounded-lg space-y-4">
          ...
          <Field label="Highlights" description="Press Enter to add each highlight">
            <TagInput
          ...
          <Field label="Tech Stack" description="Press Enter to add each technology">
            <TagInput
              value={formData.tech}
...
const CaseStudiesTab = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.fetchCaseStudies' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const resetForm = () => {
    setFormData({ metric: '', title: '', before_content: '', after_content: '' });
    setEditingId(null);
    setIsAdding(false);
  };
  ...
  const handleReorder = async (index: number, direction: 'up' | 'down') => {
    ...
    [newStudies[index], newStudies[newIndex]] = [newStudies[newIndex], newStudies[index]];
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="text-foreground">{study.before_content}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">After: </span>
                    <span className="text-foreground">{study.after_content}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="p-4 bg-muted/50 border border-border rounded-lg space-y-4">
          ...
          <Field label="Before" description="The problem/state before">
            <Textarea
              ...
              onChange={(e) => setFormData({ ...formData, before_content: e.target.value })}
          ...
          <Field label="After" description="The solution/result after">
            <Textarea
              ...
              onChange={(e) => setFormData({ ...formData, after_content: e.target.value })}
...
const timelineThemes = {
  warm: {
    ...
    segments: ['hsl(25, 15%, 30%)', 'hsl(25, 40%, 40%)', 'hsl(25, 70%, 48%)', 'hsl(25, 95%, 53%)'],
    segmentText: ['hsl(30, 10%, 95%)', 'hsl(30, 10%, 95%)', 'hsl(20, 20%, 5%)', 'hsl(20, 20%, 5%)'],
  ...
  crisp: {
    ...
    segments: ['hsl(177, 23%, 53%)', 'hsl(177, 50%, 35%)', 'hsl(177, 94%, 21%)', 'hsl(176, 62%, 18%)'],
    segmentText: ['hsl(169, 52%, 4%)', 'hsl(60, 10%, 97%)', 'hsl(60, 10%, 97%)', 'hsl(60, 10%, 97%)'],
...
const TimelinePreview = ({ entries }: { entries: TimelineEntry[] }) => {
  ...
  const getDotPosition = (dotPosition: number, entryCount: number, index: number) => {
  ...
  if (entries.length === 0) {
    return <div className="text-center py-12 text-muted-foreground">No entries to preview</div>;
  }
  ...
  return (
    <div className="bg-background rounded-lg border border-border p-6">
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                marginLeft: -9,
                marginTop: -9,
                background: expandedId === entry.id ? theme.primary : theme.progressInactive,
              }}
        ...
        {entries.map((entry) => (
          <button
            ...
            className={`text-center font-mono text-xs uppercase tracking-wider transition-colors cursor-pointer ${
              expandedId === entry.id ? '' : 'text-muted-foreground hover:text-foreground'
            }`}
            style={{ color: expandedId === entry.id ? theme.primary : undefined }}
          >
            {entry.phase}
          </button>
        ))}
      ...
      <AnimatePresence mode="wait">
        {expandedId && entries.filter(e => e.id === expandedId).map((entry) => (
          <motion.div
            key={entry.id}
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <span className="font-mono text-lg font-bold" style={{ color: theme.primary }}>{entry.label}</span>
              <span className="text-muted-foreground">—</span>
              <span className="text-foreground font-semibold">{entry.phase}</span>
            </div>
...
const TimelinesTab = () => {
  ...
  useEffect(() => {
    async function fetchTimelines() {
      try {
        ...
        captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.fetchTimelines' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    async function fetchEntries() {
      ...
      try {
        ...
        captureException(err instanceof Error ? err : new Error(String(err)), { context: 'Profile.fetchTimelineEntries' });
      } finally {
  ...
  const resetTimelineForm = () => {
    setTimelineForm({ name: '', slug: '', description: '', chart_type: 'growth' });
    setEditingTimelineId(null);
    setIsAddingTimeline(false);
  };

  const resetEntryForm = () => {
    setEntryForm({ label: '', phase: '', summary: '', content: '', dot_position: 50 });
  ...
  const handleSaveTimeline = async () => {
    try {
      if (editingTimelineId) {
        ...
        setTimelines(timelines.map((t) => (t.id === editingTimelineId ? updated : t)));
    ...
    try {
      if (editingEntryId) {
        ...
        const created = await createTimelineEntry(
          { ...entryForm, timeline_id: selectedTimeline.id, order_index: entries.length },
          supabase
        );
        setEntries([...entries, created]);
      }
  ...
  const handleReorderEntry = async (index: number, direction: 'up' | 'down') => {
    const newIndex = direction === 'up' ? index - 1 : index + 1;
    if (newIndex < 0 || newIndex >= entries.length) return;
    ...
    await reorderTimelineEntries(newEntries.map((e) => e.id), supabase);
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 p-4 bg-card border border-border rounded-lg">
        ...
        {selectedTimeline && (
          <>
            <Button variant="outline" size="sm" onClick={() => handleEditTimeline(selectedTimeline)}>
              <Edit2 className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => handleDeleteTimeline(selectedTimeline.id)}>
              <Trash2 className="w-4 h-4" />
            </Button>
          </>
        )}
        <Button variant="outline" size="sm" onClick={() => setIsAddingTimeline(true)}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Slug" description="URL-friendly identifier">
              <Input
                ...
                onChange={(e) => setTimelineForm({ ...timelineForm, slug: e.target.value })}
                placeholder="provisioning-roadmap"
              />
            </Field>
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Chart Type">
              <Select
                ...
                onChange={(e) => setTimelineForm({ ...timelineForm, chart_type: e.target.value as TimelineChartType })}
                options={chartTypeOptions}
              />
            </Field>
          </div>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={resetTimelineForm}>Cancel</Button>
      ...
      {viewMode === 'preview' && selectedTimeline && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">Exactly as it appears on the site</span>
          </div>
          {isLoadingEntries ? (
        ...
        <>
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-primary font-bold">{entry.label}</span>
                            <span className="text-muted-foreground">—</span>
                            <span className="font-medium text-foreground">{entry.phase}</span>
                          </div>
                          {entry.summary && (
          ...
          {isAddingEntry ? (
            <div className="p-4 bg-muted/50 border border-border rounded-lg space-y-4">
              ...
              <Field label="Summary" description="Brief description (shown under phase)">
                <Input
              ...
              <Field label="Content" description="Full description (shown in expanded view)">
                <Textarea
              ...
              <Field label={`Dot Position: ${entryForm.dot_position}%`} description="0 = bottom, 100 = top of chart">
                <input
                  type="range"
                  min="0"
                  max="100"
                  value={entryForm.dot_position}
                  onChange={(e) => setEntryForm({ ...entryForm, dot_position: parseInt(e.target.value) })}
                  className="w-full"
                />
              </Field>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={resetEntryForm}>Cancel</Button>
            ...
            <Button variant="outline" onClick={() => setIsAddingEntry(true)} className="w-full">
              <Plus className="w-4 h-4 mr-2" />
              Add Entry
            </Button>
          )}
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        <header className="mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Manage your personal information and content</p>
            </div>
          </div>
        </header>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'work-history' && <WorkHistoryTab key="work-history" />}
