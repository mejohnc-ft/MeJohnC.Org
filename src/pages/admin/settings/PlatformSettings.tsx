    ...
    items: [
      {
        name: "React",
        version: "18.3.1",
        reason: "Component-based UI with hooks for state management",
      },
      {
        name: "TypeScript",
        version: "5.6.2",
        reason: "Type safety and better DX with autocomplete",
      },
      {
        name: "Vite",
        version: "6.0.1",
      ...
      },
      {
        name: "Tailwind CSS",
        version: "3.4.15",
        reason: "Utility-first CSS for rapid styling",
      },
      {
        name: "Framer Motion",
        version: "11.11.17",
        reason: "Smooth, physics-based animations",
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "Supabase",
        version: "2.50.3",
        reason: "PostgreSQL with real-time, auth, and instant APIs",
      },
      {
        name: "Clerk",
        version: "5.18.0",
        reason: "Drop-in auth with social logins and session management",
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "Radix UI",
        version: "1.1.x",
        reason: "Accessible, unstyled primitives for custom components",
      },
      {
        name: "Lucide Icons",
        version: "0.460.0",
        reason: "Consistent, customizable icon set",
      },
      {
        name: "Recharts",
        version: "2.15.0",
        reason: "Composable charting for data visualization",
      },
    ],
  },
  {
    ...
    items: [
      {
        name: "ESLint",
        version: "9.13.0",
        reason: "Code quality and consistent style enforcement",
      },
      {
        name: "PostCSS",
        version: "8.4.47",
        reason: "CSS processing with autoprefixer",
      },
    ],
  },
];
...
const PlatformSettings = () => {
  ...
  const testSupabase = async () => {
    if (!supabaseUrl || !supabaseKey) {
      ...
      setSupabaseMessage("Enter URL and key");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSupabaseMessage("Connected");
      } else {
        setSupabaseStatus("error");
        setSupabaseMessage(`${res.status}`);
      }
    } catch {
      setSupabaseStatus("error");
      setSupabaseMessage("Failed");
    }
  };
  ...
  const testClerk = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        setClerkStatus("error");
        setClerkMessage("No token");
      }
    } catch {
      setClerkStatus("error");
      setClerkMessage("Failed");
    }
  };
  ...
  const testGA = async () => {
    if (!gaMeasurementId) {
      setGaStatus("error");
      setGaMessage("Enter ID");
      return;
    }
    const gaPattern = /^G-[A-Z0-9]+$/i;
    if (!gaPattern.test(gaMeasurementId.trim())) {
      setGaStatus("error");
      setGaMessage("Invalid format");
      return;
    }
    setGaStatus("success");
    setGaMessage("Valid");
  ...
  const StatusBadge = ({
    ...
    return (
      <span
        className={`text-[10px] px-1.5 py-0.5 rounded ${isSuccess ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
      >
        {isSuccess ? (
  ...
  const ConfiguredBadge = ({ configured }: { configured: boolean }) => (
    <span
      className={`text-[9px] px-1.5 py-0.5 rounded ${configured ? "bg-green-500/20 text-green-400" : "bg-muted text-muted-foreground"}`}
    >
      {configured ? "OK" : "—"}
    </span>
  );
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">
            Manage integrations, SEO, and site settings
          </p>
        </div>
            ...
            <div className="bg-card border border-border rounded-lg p-3 space-y-2">
              ...
              <div className="text-xs space-y-1">
                <div className="flex justify-between p-1.5 bg-muted/50 rounded">
                  ...
                  <span
                    className={
                      gaMeasurementId ? "text-green-400" : "text-red-400"
                    }
                  >
                ...
                {gaMeasurementId && (
                  <div className="flex justify-between p-1.5 bg-muted/50 rounded">
                    ...
                    <span className="font-mono">
                      {gaMeasurementId.slice(0, 4)}••••
                      {gaMeasurementId.slice(-3)}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-1">
                <Button
                  ...
                  variant="outline"
                  onClick={testGA}
                  disabled={gaStatus === "testing" || gaStatus === "saving"}
                  className="h-6 text-xs px-2"
                >
                  {gaStatus === "testing" ? (
                    <Loader2 className="w-3 h-3 animate-spin" />
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setSeo({
                          ...seo,
                          location: { ...seo.location, city: e.target.value },
                        })
                      }
                      placeholder="City"
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSeo({
                          ...seo,
                          location: { ...seo.location, state: e.target.value },
                        })
                      }
                      placeholder="State"
                      className="px-2 py-1.5 bg-background border border-border rounded text-sm"
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSeo({ ...seo, linkedinUrl: e.target.value })
                      }
                      className="w-full px-2 py-1.5 bg-background border border-border rounded text-sm"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={seo.githubUrl}
                      onChange={(e) =>
                        setSeo({ ...seo, githubUrl: e.target.value })
                      }
                      className="w-full px-2 py-1.5 bg-background border border-border rounded text-sm"
                ...
                <div className="pt-4">
                  <div className="flex items-center gap-2">
                    ...
                    {seoMessage && (
                      <span
                        className={`text-xs px-2 py-1 rounded ${seoStatus === "success" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                      >
                        {seoMessage}
                      </span>
                    )}
