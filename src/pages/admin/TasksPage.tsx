...
import { Plus, Search, ListTodo, Kanban } from 'lucide-react';
import { useAuthenticatedSupabase } from '@/lib/supabase';
import { getTasks, getTaskCategories, deleteTask, bulkDeleteTasks, getTaskStats } from '@/lib/task-queries';
...
const TasksPage = () => {
  ...
  async function fetchData() {
    ...
    try {
      ...
      captureException(error instanceof Error ? error : new Error(String(error)), { context: 'TasksPage.fetchData' });
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const handleDelete = async (taskId: string) => {
    ...
    try {
      ...
      captureException(error instanceof Error ? error : new Error(String(error)), { context: 'TasksPage.handleDelete' });
    }
  };

  const handleBulkDelete = async () => {
    if (!supabase || selectedTasks.length === 0 || !confirm(`Delete ${selectedTasks.length} tasks?`)) return;
    ...
    try {
      ...
      captureException(error instanceof Error ? error : new Error(String(error)), { context: 'TasksPage.handleBulkDelete' });
    }
  };
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            ...
            >
              ...
              <p className="text-2xl font-bold text-foreground">{stats.todo_count}</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-2xl font-bold text-purple-600">{stats.review_count}</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-2xl font-bold text-green-600">{stats.done_count}</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.overdue_count}</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-2xl font-bold text-orange-600">{stats.high_priority_count}</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <p className="text-2xl font-bold text-red-600 dark:text-red-400">{stats.urgent_priority_count}</p>
            </motion.div>
          </div>
        )}
