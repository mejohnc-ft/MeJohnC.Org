...
type WindowAction =
  | { type: 'OPEN_WINDOW'; payload: { appId: string; title: string; x: number; y: number; width: number; height: number } }
  | { type: 'CLOSE_WINDOW'; payload: { id: string } }
  | { type: 'FOCUS_WINDOW'; payload: { id: string } }
  | { type: 'MINIMIZE_WINDOW'; payload: { id: string } }
  | { type: 'MAXIMIZE_WINDOW'; payload: { id: string }; bounds: { width: number; height: number; topOffset: number } }
  ...
  | { type: 'RESTORE_STATE'; payload: { windows: WindowState[]; nextZIndex: number } };
  ...
  switch (action.type) {
    ...
    case 'CLOSE_WINDOW': {
      ...
      const newFocused = wasFocused
        ? remaining.filter(w => !w.minimized).sort((a, b) => b.zIndex - a.zIndex)[0]?.id ?? null
    ...
    case 'MINIMIZE_WINDOW': {
      const { id } = action.payload;
      const newFocused = state.focusedWindowId === id
        ? state.windows
      ...
      return {
        ...
        windows: state.windows.map(w =>
          w.id === id
            ? {
                ...w,
                preMaximize: w.maximized ? w.preMaximize : { x: w.x, y: w.y, width: w.width, height: w.height },
                x: 0,
                y: topOffset,
                width,
    ...
    case 'RESIZE_WINDOW': {
      ...
      return {
        ...
        windows: state.windows.map(w =>
          w.id === id
            ? { ...w, width, height, ...(x !== undefined && { x }), ...(y !== undefined && { y }), maximized: false }
            : w
        ),
      };
    }
    ...
    case 'RESTORE_STATE': {
      return {
        ...
        focusedWindowId: action.payload.windows.filter(w => !w.minimized).sort((a, b) => b.zIndex - a.zIndex)[0]?.id ?? null,
      };
    }
...
export function useWindowManager() {
  ...
  const openWindow = useCallback((appId: string, title: string, x: number, y: number, width: number, height: number) => {
    dispatch({ type: 'OPEN_WINDOW', payload: { appId, title, x, y, width, height } });
  }, []);
  ...
  const maximizeWindow = useCallback((id: string, bounds: { width: number; height: number; topOffset: number }) => {
