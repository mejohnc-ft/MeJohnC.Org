...
function mergeTimelineEntries(fetched: TimelineItem[]): TimelineItem[] {
  ...
  const fallbackLogistics = entriesForTrack(
    defaultTimelineData,
    "endpoint-logistics",
  );
...
const ProjectTimeline = ({
  ...
  const setActiveTrack = useCallback(
    (track: TimelineTrackId) => {
      ...
      const entries = entriesForTrack(timelineData, track);
      const preferredEntry =
        track === "ai-products"
          ? entries.find(
              (entry) => entry.entry_key === "ai-automation-adoption",
            )
          : entries[0];
      setExpandedId(preferredEntry?.id ?? null);
      setIsAutoPlaying(false);
      setFocusLevel("timeline");
    },
    [onTrackChange, setFocusLevel, timelineData],
  );
    ...
    async function fetchTimeline() {
      ...
      try {
        ...
        if (data && data.entries.length > 0) {
          setTimelineData(mergeTimelineEntries(data.entries.map(mapEntry)));
        }
      } catch (err) {
  ...
  const activeEntries = useMemo(
    () => entriesForTrack(timelineData, activeTrack),
    [activeTrack, timelineData],
  );
  const trackMeta =
    TIMELINE_TRACKS.find((track) => track.id === activeTrack) ??
    TIMELINE_TRACKS[0];

  useEffect(() => {
    setExpandedId((current) => {
      const stillOnTrack = activeEntries.some((item) => item.id === current);
      return stillOnTrack ? current : (activeEntries[0]?.id ?? null);
    });
  }, [activeEntries]);

  const goToNext = useCallback(() => {
    if (activeEntries.length === 0) return;
    const currentIndex = activeEntries.findIndex(
      (item) => item.id === expandedId,
    );
    const nextIndex = (currentIndex + 1) % activeEntries.length;
    setExpandedId(activeEntries[nextIndex]?.id ?? null);
    setIsAutoPlaying(false);
  }, [activeEntries, expandedId]);
  ...
  const goToPrev = useCallback(() => {
    ...
    const prevIndex =
      (currentIndex - 1 + activeEntries.length) % activeEntries.length;
    setExpandedId(activeEntries[prevIndex]?.id ?? null);
    setIsAutoPlaying(false);
  }, [activeEntries, expandedId]);
  ...
  // Auto-rotation timer (20 seconds per phase)
  useEffect(() => {
    if (!isAutoPlaying || prefersReducedMotion) return;
    const timer = setInterval(goToNext, 20000);
    return () => clearInterval(timer);
  }, [isAutoPlaying, goToNext, prefersReducedMotion]);
  ...
  return (
    <div
      ...
      role="region"
      aria-label="Career progress roadmap"
    >
      <div
        role="tablist"
        aria-label="Career progress tracks"
          ...
          return (
            <motion.button
              ...
              onKeyDown={(e) => {
                ...
                e.preventDefault();
                const index = TIMELINE_TRACKS.findIndex(
                  (item) => item.id === track.id,
                );
                const delta = e.key === "ArrowRight" ? 1 : -1;
                const next =
                  TIMELINE_TRACKS[
                    (index + delta + TIMELINE_TRACKS.length) %
                      TIMELINE_TRACKS.length
                  ];
    ...
    <div>
          ...
          return (
            <motion.button
              ...
              transition={{
                ...
                opacity: {
                  delay: prefersReducedMotion ? 0 : 0.3 + i * 0.2,
                  duration: motionDuration,
                },
              }}
      ...
      >
        <div className="flex h-12 rounded overflow-hidden">
          {timelineData.map((item, index) => (
            <motion.button
              ...
              whileHover={
                prefersReducedMotion ? undefined : { scale: 1.02, zIndex: 10 }
              }
              whileTap={prefersReducedMotion ? undefined : { scale: 0.98 }}
              role="tab"
        ...
        <button
          ...
          className="mt-2 ml-auto flex p-2 rounded border border-border hover:border-current transition-colors sm:absolute sm:-right-12 sm:top-1/2 sm:mt-0 sm:-translate-y-1/2"
      ...
      <div
        className={`grid gap-4 mb-6 ${timelineData.length <= 4 ? "grid-cols-2 md:grid-cols-4" : "grid-cols-2 md:grid-cols-" + timelineData.length}`}
      >
              ...
              style={{
                background:
                  expandedId === item.id
                    ...
                    : theme.progressInactive,
                boxShadow:
                  expandedId === item.id ? `0 0 8px ${theme.primary}` : "none",
              }}
