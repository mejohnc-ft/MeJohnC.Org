...
function parseResponse<T>(
  ...
  try {
    ...
    console.error(
      `[NpsService] Schema validation failed for ${operation}:`,
      error,
    );
    throw new Error(`Invalid data format for ${operation}`, { cause: error });
  }
}
...
export class NpsServiceSupabase implements INpsService {
  ...
  async getSurveys(
    ctx: ServiceContext,
    filters?: NPSSurveyFilters,
  ): Promise<ListResponse<NPSSurvey>> {
    ...
    return {
      data: (data || []).map((item) =>
        parseResponse(NPSSurveySchema, item, "getSurveys"),
      ),
      total: count || 0,
      hasMore: count
        ? (filters?.offset || 0) + (data?.length || 0) < count
        : false,
    };
  }
  ...
  async getResponses(
  ...
  ): Promise<ListResponse<NPSResponse>> {
    const tenantId = getTenantId(ctx);
    let query = this.client
      .from("nps_responses")
      .select("*", { count: "exact" })
    ...
    return {
      data: (data || []).map((item) =>
        parseResponse(NPSResponseSchema, item, "getResponses"),
      ),
      total: count || 0,
      hasMore: count
        ? (filters?.offset || 0) + (data?.length || 0) < count
  ...
  async getDetractors(
    ...
    return (data || []).map((item) =>
      parseResponse(NPSResponseSchema, item, "getDetractors"),
    );
  }
  ...
  ): Promise<ListResponse<NPSCampaign>> {
    const tenantId = getTenantId(ctx);
    let query = this.client
      .from("nps_campaigns")
      .select("*", { count: "exact" })
  ...
  async getAnalysis(
    ctx: ServiceContext,
    filters?: NPSAnalysisFilters,
  ): Promise<ListResponse<NPSAnalysis>> {
    const tenantId = getTenantId(ctx);
    let query = this.client
      .from("nps_analysis")
      .select("*", { count: "exact" })
  ...
  async getTrends(
    ...
    const periodBuckets = new Map<
      string,
      { scores: number[]; promoters: number; detractors: number; total: number }
    >();
