import { getApps, getAppSuites, getProjects, getSiteContent, getBlogPosts } from './supabase-queries';
...
const prefetchFunctions: Record<PrefetchKey, () => Promise<void>> = {
  projects: async () => {
    ...
    try {
      ...
      prefetchCache.set('projects', { data: { projects: projectsData, content: contentData }, timestamp: Date.now() });
    } catch (err) {
      captureException(err instanceof Error ? err : new Error(String(err)), { context: 'prefetch.projects' });
    }
  },
  ...
  software: async () => {
    ...
    try {
      ...
      prefetchCache.set('software', { data: { apps: appsData, suites: suitesData }, timestamp: Date.now() });
    } catch (err) {
      captureException(err instanceof Error ? err : new Error(String(err)), { context: 'prefetch.software' });
    }
  },
  ...
  content: async () => {
    ...
    try {
      ...
      prefetchCache.set('content', { data: { ghostPosts, localPosts }, timestamp: Date.now() });
    } catch (err) {
      captureException(err instanceof Error ? err : new Error(String(err)), { context: 'prefetch.content' });
    }
  },
};
