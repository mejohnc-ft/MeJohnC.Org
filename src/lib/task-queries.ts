...
export async function getTaskCategories(client: SupabaseClient = getSupabase()) {
...
export async function getTaskCategoryBySlug(slug: string, client: SupabaseClient = getSupabase()) {
  const { data, error } = await client
    .from('task_categories')
    .select('*')
    .eq('slug', slug)
...
export async function getTaskCategoryById(id: string, client: SupabaseClient = getSupabase()) {
  const { data, error } = await client
    .from('task_categories')
    .select('*')
    .eq('id', id)
...
export async function deleteTaskCategory(id: string, client: SupabaseClient = supabase) {
...
export async function getTaskById(id: string, client: SupabaseClient = getSupabase()) {
  const { data, error } = await client
    .from('tasks')
    .select('*, category:task_categories(*)')
    .eq('id', id)
...
export async function getTasksByStatus(status: Task['status'], client: SupabaseClient = getSupabase()) {
...
export async function getTasksByCategory(categoryId: string, client: SupabaseClient = getSupabase()) {
  return getTasks({ categoryId }, client);
}

export async function getSubtasks(parentTaskId: string, client: SupabaseClient = getSupabase()) {
  const { data, error } = await client
    .from('tasks')
    .select('*, category:task_categories(*)')
...
export async function createTask(
  task: Omit<Task, 'id' | 'created_at' | 'updated_at' | 'completed_at' | 'is_overdue'>,
  client: SupabaseClient = supabase
) {
  const { data, error } = await client
    .from('tasks')
...
export async function bulkDeleteTasks(ids: string[], client: SupabaseClient = supabase) {
  const { error } = await client
    .from('tasks')
    .delete()
    .in('id', ids);
...
export async function getTaskComments(taskId: string, client: SupabaseClient = getSupabase()) {
  const { data, error } = await client
    .from('task_comments')
    .select('*')
    .eq('task_id', taskId)
...
export async function deleteTaskComment(id: string, client: SupabaseClient = supabase) {
  const { error } = await client
    .from('task_comments')
    .delete()
    .eq('id', id);
...
export async function getTaskReminders(taskId: string, client: SupabaseClient = getSupabase()) {
  const { data, error } = await client
    .from('task_reminders')
    .select('*')
    .eq('task_id', taskId)
...
export async function getPendingReminders(client: SupabaseClient = getSupabase()) {
  const { data, error } = await client
    .from('task_reminders')
    .select('*')
    .eq('is_sent', false)
...
export async function markReminderSent(id: string, client: SupabaseClient = supabase) {
  const { data, error } = await client
    .from('task_reminders')
...
export async function deleteTaskReminder(id: string, client: SupabaseClient = supabase) {
  const { error } = await client
    .from('task_reminders')
    .delete()
    .eq('id', id);
...
export async function getTaskStats(client: SupabaseClient = getSupabase()): Promise<TaskStats> {
  const [
    totalResult,
    todoResult,
    inProgressResult,
    ...
    client.from('tasks').select('id', { count: 'exact', head: true }).eq('status', 'todo'),
    client.from('tasks').select('id', { count: 'exact', head: true }).eq('status', 'in_progress'),
    client.from('tasks').select('id', { count: 'exact', head: true }).eq('status', 'review'),
    client.from('tasks').select('id', { count: 'exact', head: true }).eq('status', 'done'),
    client.from('tasks').select('id', { count: 'exact', head: true }).eq('is_overdue', true),
    client.from('tasks').select('id', { count: 'exact', head: true }).eq('priority', 'high'),
    client.from('tasks').select('id', { count: 'exact', head: true }).eq('priority', 'urgent'),
  ]);
