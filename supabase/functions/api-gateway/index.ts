...
import { canPerformAction, resolveRoute, ACTION_CAPABILITY_MAP } from '../_shared/capabilities.ts'
...
async function dispatchAction(
  ...
  switch (route.type) {
    case 'workflow': {
      ...
      if (!workflowId) {
        return { status: 400, data: { error: 'workflow_id required for workflow actions' } }
      }
      ...
      if (!schedulerSecret) {
        ...
        return { status: 500, data: { error: 'Internal dispatch configuration error' } }
      }
    ...
    case 'agent': {
      ...
      if (action === 'agent.status') {
        const { data, error } = await supabase
          .from('agents')
          .select('id, name, type, status, capabilities, health_status, last_seen_at')
          .eq('status', 'active')
      ...
      return { status: 400, data: { error: `Unknown agent action: ${action}` } }
    }
    ...
    case 'integration': {
      ...
      return { status: 400, data: { error: `Unknown integration action: ${action}` } }
    }
    ...
    case 'system': {
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/${route.handler}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-scheduler-secret': schedulerSecret,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (signatureHeader && agent.metadata) {
      ...
      if (agentRow?.signing_secret_encrypted) {
        ...
        if (!sigResult.valid) {
          ...
          return new Response(
            JSON.stringify({ error: 'Signature verification failed', message: sigResult.error }),
    ...
    const response: GatewayResponse = {
      ...
      error: result.status >= 400 ? (result.data as Record<string, string>)?.error : undefined,
      meta: {
        agent_id: agent.id,
        action,
        duration_ms: duration,
