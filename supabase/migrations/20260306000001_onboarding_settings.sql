-- Onboarding Settings Extension
-- Issue: #316
--
-- Extends update_tenant_settings RPC with onboarding_complete and onboarding_step
-- parameters so the customer onboarding wizard can persist progress.

-- Drop the old 5-param signature
DROP FUNCTION IF EXISTS app.update_tenant_settings(JSONB, JSONB, JSONB, JSONB, JSONB);
...
BEGIN
  ...
  IF p_onboarding_complete IS NOT NULL THEN
    v_patch := v_patch || jsonb_build_object('onboarding_complete', p_onboarding_complete);
  END IF;

  IF p_onboarding_step IS NOT NULL THEN
    v_patch := v_patch || jsonb_build_object('onboarding_step', p_onboarding_step);
  END IF;
...
REVOKE ALL ON FUNCTION app.update_tenant_settings(JSONB, JSONB, JSONB, JSONB, JSONB, BOOLEAN, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION app.update_tenant_settings(JSONB, JSONB, JSONB, JSONB, JSONB, BOOLEAN, INT) TO authenticated;
